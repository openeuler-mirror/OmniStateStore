...
3. 通过使用merge替换状态RMW，减少Join算子的状态更新开销；
4. 通过双流Join数据缓存算法，减少StreamJoinOperator的状态范围查询次数；
5. 通过动态Filter技术，过滤冗余状态查询操作；
...
#### 版本说明
1. 适用于Flink1.16.3和RocksDB6.20.3, 版本迁移需要进行轻量级代码适配;
...
```
3. 检查编译成功的软件包。
编译成功则在目录java/target/下存在：
OmniStateStore软件包BoostKit-omniruntime-omniStateStore_1.x.x.zip，其中包含flink-alg-falcon.jar和librocksdb.so。

# 安装部署
#### 软件安装
1. 登录待安装节点，上传软件包BoostKit-omniruntime-omniStateStore_1.x.x.zip到“${FLINK_HOME}/lib/”子目录下；
2. 解压软件包, 得到flink-alg-falcon.jar和librocksdb.so
3. 将librocksdb.so拷贝到LD_LIBRARY_PATH目录下
```
unzip BoostKit-omniruntime-omniStateStore_1.x.x.zip
rm -rf BoostKit-omniruntime-omniStateStore_1.x.x.zip
mv librocksdb.so.x /usr/local/lib
```

#### 软件启动
1. 配置参数项：
根据业务使用情况和待安装部署的环境设置Flink的conf子目录下flink-conf.yaml中的相关配置项, 示例配置参数详见本工程conf/flink-conf.yaml；
2. 配置LD_LIBRARY_PATH为：$LD_LIBRARY_PATH:$JAVA_HOME/lib:$JAVA_HOME/jre/lib/aarch64:$JAVA_HOME/jre/lib/aarch64/server:/usr/local/lib
3. 启动Flink任务，查看日志中的配置项，检查配置是否成功；
4. 执行Nexmark0.3 Q4用例，观察到Task Manager日志中打印“[FALCON]”关键词，说明OmniStateStore使能成功。

#### 软件卸载
1. 将“${FLINK_HOME}/lib/”目录下的flink-alg-falcon.jar删除；
2. 将librocksdb.so从/usr/local/lib目录移除，并恢复环境变量LD_LIBRARY_PATH；
3. 删除“${FLINK_HOME}/conf/flink-conf.yaml”中的相关配置参数。
