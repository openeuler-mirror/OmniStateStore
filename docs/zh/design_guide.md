...
**- 在随机读写场景下，使用bloom-filer可能导致系统性能严重下降。** 例如，在随机读写场景下，RocksDB的SSTable将频繁的进行compaction操作，从而导致内存的filter频繁失效。此时，需要频繁的根据SSTable创建新的filter。在nexmark0.3-Q15用例中，打开bloom-filter将导致filter miss率上升10X，进而导致系统性能下降10X。<br>
