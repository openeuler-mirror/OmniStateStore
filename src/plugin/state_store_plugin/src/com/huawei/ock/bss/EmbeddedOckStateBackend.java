...
public class EmbeddedOckStateBackend extends AbstractManagedMemoryStateBackend implements ConfigurableStateBackend {
    ...
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(Environment env, JobID jobID,
        ...
        @Nonnull Collection<KeyedStateHandle> stateHandles, CloseableRegistry cancelStreamRegistry) throws IOException {
        LOG.info("enter OckStateBackend createKeyedStateBackend with default managedMemoryFraction.");
        return createKeyedStateBackend(env, jobID, operatorIdentifier, keySerializer, numberOfKeyGroups, keyGroupRange,
            kvStateRegistry, ttlTimeProvider, metricGroup, stateHandles, cancelStreamRegistry, 1.0D);
    }
     ...
     *
     * @param env                  环境
     * @param operatorIdentifier   标识符
     * @param stateHandles         stateHandles
     * @param cancelStreamRegistry closeableRegistry
    ...
    public OperatorStateBackend createOperatorStateBackend(Environment env, String operatorIdentifier,
        @Nonnull Collection<OperatorStateHandle> stateHandles, CloseableRegistry cancelStreamRegistry)
        throws Exception {
        LOG.info("create OperatorStateBackend of operator:{}", LogSanitizer.sanitize(operatorIdentifier));
        return new DefaultOperatorStateBackendBuilder(env.getUserCodeClassLoader().asClassLoader(),
            env.getExecutionConfig(), true, stateHandles, cancelStreamRegistry).build();
    }
