...
public class BoostIncrementalRestoreOperation<K> extends AbstractBoostIncrementalRestoreOperation<K> {
    ...
    public BoostIncrementalRestoreOperation(String jobID, String operatorIdentifier, KeyGroupRange keyGroupRange,
        ...
        CloseableRegistry cancelStreamRegistry, ClassLoader userCodeClassLoader,
        ...
        super(jobID, operatorIdentifier, keyGroupRange, db, keyGroupPrefixBytes, numberOfTransferringThreads,
            cancelStreamRegistry, userCodeClassLoader, priorityQueueSetFactory, registeredKvStateMetaInfos,
            keyedStateDescriptorMap, nsKeyedStateDescriptorMap, registeredPQStates, tables, keySerializerProvider,
            instanceBasePath, restoreStateHandles);
    ...
    public BoostRestoreResult restore() throws Exception {
        ...
        final KeyedStateHandle firstStateHandle = restoreStateHandles.iterator().next();
        if (!Objects.equals(keyGroupRange, firstStateHandle.getKeyGroupRange()) || restoreStateHandles.size() > 1) {
            restoreStateHandles.remove(firstStateHandle);
            restoreWithRescaling(restoreStateHandles, firstStateHandle);
            LOG.info("Finished restoring with rescaling.");
        } else {
            restoreWithoutRescaling(firstStateHandle);
    ...
    protected IncrementalLocalKeyedStateHandle transferRemoteStateToLocalDirectory(
        ...
        for (HandleAndLocalPath handleAndLocalPath : remoteKeyedStateHandle.getPrivateState()) {
            final String fileName = handleAndLocalPath.getLocalPath();
            // 记录当前文件的本地文件地址，如果是共享文件，记录的只是文件名，如果是独享的文件，记录的是当前本地checkpoint的绝对路径。
            if (fileName.endsWith("metadata") || fileName.endsWith("dat")) {
                restoreLocalPath = tmpPathToRestore.resolve(handleAndLocalPath.getLocalPath()).getParent();
                break;
            }
        }
        ...
        return new IncrementalLocalKeyedStateHandle(remoteKeyedStateHandle.getBackendIdentifier(),
            remoteKeyedStateHandle.getCheckpointId(), new DirectoryStateHandle(restoreLocalPath, 1),
            remoteKeyedStateHandle.getKeyGroupRange(), remoteKeyedStateHandle.getMetaDataStateHandle(),
