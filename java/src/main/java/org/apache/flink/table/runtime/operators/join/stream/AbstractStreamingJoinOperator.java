...
public abstract class AbstractStreamingJoinOperator extends AbstractStreamOperator<RowData>
    ...
    protected static final class AssociatedRecords {
        ...
        /**
         * Creates an {@link AssociatedRecords} which represents the records whose key is same as the input row.
         * Then, other join condition will be checked during falcon miniBatch join.
         * @brief FALCON implementation
         * @param otherSideStateView other side rocksdb instance
         * @return records whose key is same as the input row
         */
        public static AssociatedRecords innerJoinOfSameKey(JoinRecordStateView otherSideStateView) throws Exception {
            List<OuterRecord> associations = new ArrayList<>();
            // range query records whose key is same as input record from other side rocksdb instance.
            Iterable<RowData> records = otherSideStateView.getRecords();
            for (RowData record : records) {
                associations.add(new OuterRecord(record, -1));
            }
            // transform into AssociatedRecords type
