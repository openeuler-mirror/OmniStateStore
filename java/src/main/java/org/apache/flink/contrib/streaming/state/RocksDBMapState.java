...
import static com.huawei.falcon.state.RocksDBRuntimeOption.custom_factory;
import static com.huawei.falcon.state.RocksDBRuntimeOption.prefixLength;
...
class RocksDBMapState<K, N, UK, UV> extends AbstractRocksDBState<K, N, Map<UK, UV>>
        implements InternalMapState<K, N, UK, UV>, MergeableState<UK, UV> {

    private static final Logger LOG = LoggerFactory.getLogger(RocksDBMapState.class);

    private final boolean supportsNullValue;

    private final boolean factoryValid;

    private final boolean useRangeFilter;

    private final int currentPrefixLength;

    private final ReadOptions mapStateReadOption;
    ...
    private RocksDBMapState(
        ...
        this(columnFamily, namespaceSerializer, valueSerializer, defaultValue, backend, true);
    }
    ...
    private RocksDBMapState(
        ...
        this.factoryValid = config.getOptional(custom_factory).isPresent() &&
                config.get(custom_factory).equals("com.huawei.falcon.state.RocksDBOptOptionsFactory");
        this.useRangeFilter = config.get(USE_RANGE_FILTER);
        this.currentPrefixLength = config.get(prefixLength);

        this.mapStateReadOption = new ReadOptions(backend.getReadOptions());
    }
    ...
    public UV get(UK userKey) throws IOException, RocksDBException {
        ...
        return (rawValueBytes == null
                ...
                : deserializeUserValue(
                        dataInputView, rawValueBytes, userValueSerializer, supportsNullValue));
    }
    ...
    public void put(UK userKey, UV userValue) throws IOException, RocksDBException {
        ...
        byte[] rawValueBytes =
                supportsNullValue
                        ? serializeValueNullSensitive(userValue, userValueSerializer)
                        : serializeValue(userValue, userValueSerializer);

        backend.db.put(columnFamily, writeOptions, rawKeyBytes, rawValueBytes);
    }

    @Override
    public void merge(UK key, UV operand) throws IOException, RocksDBException {
        byte[] rawKeyBytes =
                serializeCurrentKeyWithGroupAndNamespacePlusUserKey(key, userKeySerializer);
        byte[] rawOperandBytes =
                supportsNullValue
                        ? serializeValueNullSensitive(operand, userValueSerializer)
                        : serializeValue(operand, userValueSerializer);
        backend.db.merge(columnFamily, writeOptions, rawKeyBytes, rawOperandBytes);
    }
    ...
    public void clear() {
        ...
        if (useRangeFilter && factoryValid && keyPrefixBytes.length >= currentPrefixLength) {
            mapStateReadOption.setTotalOrderSeek(false); // only used for the first
        } else {
            mapStateReadOption.setTotalOrderSeek(true);
        }

        try (RocksIteratorWrapper iterator =
                        RocksDBOperationUtils.getRocksIterator(
                                backend.db, columnFamily, mapStateReadOption);
    ...
    private abstract class RocksDBMapIterator<T> implements Iterator<T> {
        ...
        private void loadCache() {
            ...
            if (useRangeFilter && factoryValid && currentEntry == null && keyPrefixBytes.length >= currentPrefixLength) {
                mapStateReadOption.setTotalOrderSeek(false); // only used for the first
            } else {
                mapStateReadOption.setTotalOrderSeek(true);
            }
            ...
            try (RocksIteratorWrapper iterator =
                    RocksDBOperationUtils.getRocksIterator(
                            db, columnFamily, mapStateReadOption)) {
    ...
    static <UK, UV, K, N, SV, S extends State, IS extends S> IS create(
        ...
        return (IS)
                new RocksDBMapState<>(
                        ...
                        backend,
                        ((MapStateDescriptor<UK, UV>) stateDesc).supportsNullValue());
    }
