...
import static com.huawei.falcon.state.RocksDBRuntimeOption.optimizeValueOption;
import static com.huawei.falcon.state.RocksDBRuntimeOption.optimizeMapOption;
import static org.apache.flink.api.common.state.StateDescriptor.Type.VALUE;
import static org.apache.flink.api.common.state.StateDescriptor.Type.MAP;
...
public class RocksDBOperationUtils {
    ...
    public static ColumnFamilyDescriptor createColumnFamilyDescriptor(
            ...
            @Nullable RocksDbTtlCompactFiltersManager ttlCompactFiltersManager,
            @Nullable Long writeBufferManagerCapacity) {
        String mergeOperatorName = MERGE_OPERATOR_NAME;
        if (metaInfoBase instanceof RegisteredKeyValueStateBackendMetaInfo) {
            String mergeOperatorNameFromMetaInfo =
                    ((RegisteredKeyValueStateBackendMetaInfo) metaInfoBase).getMergeOperatorName();
            mergeOperatorName =
                    mergeOperatorNameFromMetaInfo == null
                            ? MERGE_OPERATOR_NAME
                            : mergeOperatorNameFromMetaInfo;
        }
    ...
    public static ColumnFamilyDescriptor createColumnFamilyDescriptor(
            ...
            @Nullable RocksDbTtlCompactFiltersManager ttlCompactFiltersManager,
            @Nullable Long writeBufferManagerCapacity,
            @Nonnull String mergeOperatorName) {

        ColumnFamilyOptions options =
                createColumnFamilyOptions(
                        columnFamilyOptionsFactory, metaInfoBase.getName(), mergeOperatorName);
        if (metaInfoBase instanceof RegisteredKeyValueStateBackendMetaInfo) {
            if (((RegisteredKeyValueStateBackendMetaInfo<?, ?>) metaInfoBase).getStateType() == VALUE) {
                optimizeValueOption(metaInfoBase, options);
            } else if (((RegisteredKeyValueStateBackendMetaInfo<?, ?>) metaInfoBase).getStateType() == MAP) {
                optimizeMapOption(metaInfoBase, options);
            }
        }
    ...
    public static ColumnFamilyOptions createColumnFamilyOptions(
        ...
        return createColumnFamilyOptions(
                columnFamilyOptionsFactory, stateName, MERGE_OPERATOR_NAME);
    }
    ...
    public static ColumnFamilyOptions createColumnFamilyOptions(
        ...
        // ensure that we use the right merge operator, because other code relies on this
        return columnFamilyOptionsFactory.apply(stateName).setMergeOperatorName(mergeOperatorName);
    }
