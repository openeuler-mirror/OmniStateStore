...
/** Type serializer for {@code Long} compatible with RocksDb's Uint64Add merge operator. */
@Internal
public final class UInt64RocksDbSerializer extends TypeSerializerSingleton<Long> {
    ...
    public static final UInt64RocksDbSerializer INSTANCE = new UInt64RocksDbSerializer();
    ...
    public void serialize(Long record, DataOutputView target) throws IOException {
        if (LITTLE_ENDIAN) {
            target.writeLong(Long.reverseBytes(record));
        } else {
    ...
    public TypeSerializerSnapshot<Long> snapshotConfiguration() {
        return new UInt64RocksDbSerializerSnapshot();
    }
    ...
    public static final class UInt64RocksDbSerializerSnapshot
            extends SimpleTypeSerializerSnapshot<Long> {

        public UInt64RocksDbSerializerSnapshot() {
            super(() -> INSTANCE);
        }
    }
}
